...
## 31. Planning's Task Picker Only Offers Each Role Its Own Kind of Task

A product-owner ask, again after hands-on use of the day-level Planning grid (§22): when planning a **Nester**'s day, only IFC-completed work should be offered — and, worded as a reminder rather than a new ask, a **Detailer**'s task should keep showing its real stage (IFA while drafting, IFC once IFA's issued, and — once batched — BOM as the room's actual production category). That second half was already exactly how the grid behaves: `categorizeRoomStage`/`categorizeBatchStatus` (`lib/plan-colors.ts`, §22/§24.2) already colour and label a Detailer's Room task "IFA" or "IFC" off its live `workflow_stage.key`, and a Nester's Batch task "BOM"/"Nesting"/"Complete" off the batch's own `status` — verified live rather than just re-confirmed by reading the code (see below). The first half was a real gap: the "+ add task" picker (`TaskPickerDialog.tsx`) offered every eligible Room *and* every eligible Batch to *any* row, Detailer or Nester alike, with nothing tying the task's type to who it was being planned for.

That gap was worse than a cosmetic mismatch. `plan_service.create_entry`'s own docstring spells out why: planning a Room task onto someone also sets `Room.assigned_detailer_id` to that same person — so picking a Batch for a Detailer was merely useless, but picking a Room for a Nester would have silently made a Nester into a room's real detailer assignment, corrupting My Work/room-ownership data for a room they'd have no actual actions available on. Since a Batch can only ever be created from rooms that already reached `ifc_issued` (§21.4's own eligibility check), "only Batches" and "only IFC-completed work" are the same restriction — a Nester's task list needed no separate stage filter of its own.

**Backend**: `app/api/routes/planning.py::_assert_task_matches_user_role(user, room, batch)` — a Room task requires `UserRole.DETAILER`, a Batch task requires `UserRole.NESTER`, 400 otherwise. Called from both `create_plan_entry` (the "+ add task" picker) and `update_plan_entry` (the grid's drag-and-drop / "move" fallback — `PlanEntryUpdate` deliberately has no `room_id`/`batch_id` of its own, per its existing docstring, so only the *person* can change on a move, and this re-checks that new person against the entry's unchanged task type). Deliberately has no admin/Team-Leader bypass the way `require_role()` does — this isn't "who's allowed to perform the action," it's "does this task/person pairing even make sense," and it wouldn't make sense for any role.

**Frontend**: `TaskPickerDialog.tsx` filters its own `rooms`/`batches` lists by `detailer.role` before rendering — a Nester's picker computes `batches` only (rooms forced empty), a Detailer's computes `rooms` only — so the picker never offers something the backend would reject anyway. `PLANNABLE_ROLES` (`app/(app)/planning/page.tsx`, unchanged) already limited Planning's *rows* to Detailer/Nester; this is the same split applied to what each row's row can actually be assigned.

Verified live against the real seeded database: as Team Leader, `POST /planning/entries` with a `room_id` targeted at the Nester account gets 400 ("A room task can only be planned for a Detailer"); the same call with a `batch_id` targeted at a Detailer account gets 400 ("A batch task can only be planned for a Nester"); the correct pairing in both directions (Room→Detailer, Batch→Nester) succeeds with 201; dragging (`PATCH .../entries/{id}` with a new `user_id`) an existing Room-task entry onto the Nester and an existing Batch-task entry onto a Detailer both correctly 400 the same way. On the frontend, opening the picker for a Detailer's row shows a "Rooms" section only (ten real eligible rooms, colour-coded IFA/IFC per their live stage); opening it for the Nester's row shows a "Batches" section only (the one real eligible batch, in Nesting, colour-coded accordingly) — confirming the grid's existing IFA→IFC→BOM/Nesting colour progression (§22/§24.2) was already correct and needed no change.

## 32. Named Review/Client-Response Actions, Replacing the Generic "Move Stage" Picker for Management and Client-Outcome Roles

A product-owner walkthrough of the full IFA/IFC cycle end to end, confirming (and, in two places, correcting) the review flow §21/§29 built: **Start IFA** is the detailer's first click; they either **submit it for internal review** or put it **On Hold** with a reason; the Team Leader checks it, with a place to **write feedback if there are issues**; once any changes are made and resubmitted, the Team Leader can **mark IFA complete**, which is what actually notifies the Project Manager; once the client's response comes back, the **PM marks it Approved** (or records markups); only then does **Start IFC** appear for the detailer. Comparing this against §30's very last change surfaced one real naming mistake worth correcting, and against the room detail page's actual UI surfaced one real UX gap worth closing — both fixed here.

**The naming correction**: §30 relabelled the detailer's own submit-for-review click "IFA Complete"/"IFC Complete". That was wrong — submitting a draft for review isn't "complete," it's *submitted*; "complete" is what the Team Leader says once they've actually checked it and it passed. `RoomActions.tsx::nextStageLabel` (shared by My Work and the room detail page's detailer action set) now returns "Submit IFA Review"/"Submit IFC Review" again — the exact wording §29 originally used, restored once the right owner for "Complete" was identified. `startLabel` ("Start IFA"/"Start IFC") was correct all along and is unchanged.

**The UX gap**: the Team Leader's and the Project Manager/Manager's own actions were still a generic `StageTransitionCard` — a "Move to" dropdown (offering whichever 1-2 stages `allowedStageTransitions`, §29, actually allows), an "Outcome" selector shown only at `ifa_issued`, a free-text note, and one "Move stage" submit button. Functionally complete, but nothing like the named, one-click clarity the detailer's own action set (§20/§23.3/§30) or the Batch page's `LifecycleControls` (§30) already had. `app/(app)/rooms/[id]/page.tsx`'s `StageTransitionCard` is now a dispatcher over three presentations, chosen by the room's current stage key:

- **`ReviewGateCard`** — at `ifa_internal_review`/`ifc_internal_review` (the `_MANAGEMENT` tier: Manager, Team Leader, Admin bypass). Two buttons: **"Mark IFA/IFC Complete"** (approves, moves to `*_issued` — this is the click that fires the Project Manager notification, §29/§21) and **"Send Back for Changes"** (moves back to `*_drafted`). A single "Feedback for the detailer" textarea sits above both — the same free-text `note` the backend has always accepted on any transition (`RoomStageEvent.note`), just now **required** specifically when sending it back (checked client-side before the API call; "changes needed" with no explanation isn't actionable), and optional when marking complete.
- **`ClientOutcomeCard`** — at `ifa_issued`/`ifc_issued` (the `_CLIENT_OUTCOME` tier: Manager, Team Leader, Project Manager, Admin bypass). Two buttons, **"Approved"** and **"Markups Required"**, recording a real `StageTransitionOutcome` rather than leaving it as an easy-to-skip optional dropdown — "Approved" folds in an "Approved with Comments" checkbox (both outcomes move to the same target stage; only the recorded outcome value differs, so a three-way stage picker was never the right shape), and "Markups Required" requires a note the same way `ReviewGateCard`'s send-back does. Deliberately extends the *same* treatment to `ifc_issued→complete`/`ifc_revision` that `ifa_issued` already had — that pair of transitions has existed since §29, but the old card's `isDecisionPoint` flag only ever showed the Outcome field at `ifa_issued`, so a Manager/Team Leader/PM moving a room from `ifc_issued` to `complete` could previously do so without the UI ever offering to record what the client said. No backend change was needed for this half — the outcome field already existed on the transition endpoint; only the frontend previously failed to surface it there.
- **`GenericStageTransitionCard`** — the plain dropdown-plus-note picker, unchanged, kept only for what's left over: in practice, an Admin/Team Leader bypass role manually working a Detailer-tier move (e.g. redrafting after a Revision) from a non-Detailer account — every real `_MANAGEMENT`/`_CLIENT_OUTCOME` transition is already covered by one of the two named cards above, so this fallback is reached rarely and only for genuinely edge-case bypass usage.

Verified live end to end against the real seeded database, working the whole cycle the product owner described in one pass: a Detailer submits IFA for review (200, notification queued for the Team Leader per §29); the Team Leader sends it back with feedback ("Door swing on the pantry is wrong…", 200, note recorded on the `RoomStageEvent`); the Detailer resubmits (200); the Team Leader marks IFA complete (200, and the Project Manager notification — `IFA_READY` — is confirmed present in the database); the Project Manager marks it Approved with `outcome: "approved"` (200, room lands on `ifc_drafted`); the Detailer's room page now shows "Start IFC" / "Submit IFC Review" / "On Hold". On the frontend specifically: the Team Leader's card at `ifa_internal_review` renders exactly "Mark IFA Complete" / "Send Back for Changes" under a "Feedback for the detailer" textarea; the PM's card at `ifa_issued` renders "Approved" / "Markups Required" with the "Approved with Comments" checkbox under "What did the client say about this IFA package?"; clicking "Markups Required" with the notes field empty shows an inline error ("Add a note on what the client wants changed before recording markups") and — confirmed against the API directly — does **not** call the transition endpoint, leaving the room's stage unchanged.
